...
--
-- Host: 127.0.0.1    Database: sharkdb
-- ------------------------------------------------------
...
CREATE DEFINER=`root`@`localhost` FUNCTION `moreDetections`(shark1 varchar(64), shark2 varchar(64)) RETURNS int
    ...
    DETERMINISTIC
BEGIN
DECLARE shark1_var varchar(64);
DECLARE shark2_var varchar(64);
    SELECT sum(detections) INTO shark1_var FROM shark WHERE name = shark1;
    SELECT sum(detections) INTO shark2_var FROM shark WHERE name = shark2;
    if shark1_var > shark2_var then return 1;
    elseif shark1_var < shark2_var then return -1;
    else return 0;
    end if;
END ;;
DELIMITER ;
...
CREATE DEFINER=`root`@`localhost` FUNCTION `mostAttacks`() RETURNS int
    ...
    DETERMINISTIC
BEGIN
	DECLARE mostAttacks_var INT;
    SELECT location INTO mostAttacks_var FROM attack
    GROUP BY location ORDER BY count(location) DESC limit 1;
RETURN mostAttacks_var;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` FUNCTION `numSharkWithLen`(length_p INT) RETURNS int
    READS SQL DATA
    DETERMINISTIC
BEGIN
	DECLARE numSharkWithLen_var INT;
    SELECT count(sid) INTO numSharkWithLen_var FROM shark
    WHERE length >= length_p;
RETURN numSharkWithLen_var;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `allReceivers`(IN town_v varchar(64), IN state_v varchar(2))
BEGIN
SELECT r.*, t.town, t.state FROM receiver as r join township as t 
on t.tid = r.location
WHERE town = town_v and state = state_v;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `sharkLenGTE`(IN length_p INT)
BEGIN
SELECT * FROM shark 
WHERE length > length_p;
END ;;
...
-- Dump completed on 2022-06-22 16:28:12
